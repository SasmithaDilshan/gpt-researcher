...
export const getHost = async ({ purpose }: GetHostParams = {}): Promise<HostResponse> => {
  ...
  const acc = process.env.CHOREO_GPT_RESEARCHER_CONNECTION_TOKEN;
  const ws = new WebSocket(`${process.env.CHOREO_GPT_RESEARCHER_CONNECTION_SERVICEURL}/ws`, ['choreo-oauth2-token',`${accessToken}`]);
